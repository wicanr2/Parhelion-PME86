...
// 分三塊，來源不同：
//
// **`0x00`–`0x22`：直譯器自己的檔頭。** `SYSTEM.PME.86` 的前 18 個 word
// 會被原封不動搬進這裡（只有 `0x06` 由 bootstrap 改寫成開機參數區的位址），
// 內容是直譯器對作業系統的自我介紹：
//
//	0x08  SYSCOM 的位址（0x00E6）
//	0x0A  區域資料基底在狀態區的偏移（0x26）
//	0x0C  全域資料基底的偏移（0x28）
//	0x0E  目前 E_Rec 的偏移（0x3E）
//	0x12–0x18  四個原生回呼的進入點
//	0x1A–0x22  其他直譯器碼位址
//
// 前四項我們自己也有同樣的東西（就是這個檔案裡的那幾個常數）；
// **後面那些是 8086 機器碼的位址，Go 宿主沒有對應的東西**——
// 作業系統只在 `NAT` 呼叫時才用得到它們，而 `NAT` 本來就做不到。
// 照抄是為了讓開機時讀到那幾格的 p-code 走同一條路。
//
// **`0x00E6` 起：SYSCOM。** 單元數、記憶體上緣、裝置參數。
// 幾個值在 `PSYSTEM.COM` 裡是立即數（`000E` 在 `0AAD`、`0009` 在 `0ADC`、
// `0050` 在 `0968`），也就是這台主機的組態。
//
// **`0x0140` 起：開機參數。** 用途還沒逐項解出來。
var bootWords = map[uint16]uint16{
	// 直譯器檔頭。前三個 word 的用途還沒解。
	0x0000: 0x3A04, 0x0004: 0x0C9B,
	0x0006: 0x0140, // bootstrap 改寫成開機參數區的位址
