...
[English](segment-switching.en.md) ｜ [日本語](segment-switching.ja.md) ｜ **繁體中文**
...
`CXG` 在真的切段之前會先問一句「這一支是不是宿主的機器碼」——
段號為 1 時查 `0x1F56` 起那張 48 格的表，查得到就直接跳進機器碼，
根本不換段。

那張表就是「哪些事不是 p-code」的完整清單，
逐支的內容見[段 1 的內嵌原生程序](native-intrinsics.md)。
...
- `0x1bee` 讀的指標相對 `ss`。原本從碼上看像是相對 `es`，
  實測不是——**這一條以量到的為準**。
- 內嵌程序那張表的逐支內容移到[段 1 的內嵌原生程序](native-intrinsics.md)。
